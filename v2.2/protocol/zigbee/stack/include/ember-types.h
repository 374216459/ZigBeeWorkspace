...
/**
 * @brief SubGhz channels bitmasks for pages 28, 30, 31
 */
...
/**
 * @brief SubGhz channels bitmasks for page 29
 */
...
/**
 * @brief Page bitmask of all channels.
 */
#define EMBER_ALL_CHANNEL_PAGE_MASK 0xF8000000UL

/**
 * @brief Page-channel mask for a given page and channel mask
 */
...
/**
 * @brief Page-channel mask for a given page and channel number
 */
...
/**
 * @brief Value of channel page when no page in use.
 */
...
/**
 * @brief options to allow/disallow rejoins using deafult link key
 */
#ifdef DOXYGEN_SHOULD_SKIP_THIS
...
#endif
{
  ...
  EMBER_ACTIVE_SCAN,
  /** Fake scan that is used to turn off the radio. */
  EMBER_START_RADIO_OFF_SCAN,
  /** Green power channel delivery scan. */
...
#endif
{
  ...
  //temporarily added back until base change is merged
 ...
 * device (2.4Ghz or SubGhz) to retrieve radio parameters.
 */
typedef struct {
  int8_t radioTxPower;
  uint8_t radioPage;
...
/** @brief This struct contains the data of child nodes
 *
 */
typedef struct {
  EmberEUI64 eui64;
...
typedef struct {
  uint8_t data;   // for now might be #of retries or other values that previously were assigned to data
...
#else
typedef uint8_t EmberLinkKeyRequestPolicy;
enum
#endif
{
...
/**
 * @brief defines the max number of counters that we specify as reporting either
 * on 2.4Ghz or SubGhz
 */
#define EMBER_MAX_MAC_ONLY_COUNTERS 33

/**
 * @brief defines the max number of phys on our boards for now
 *
 */
#define EMBER_MAX_NUM_PHYS 2

/**
 * @brief Phy index for 2.4Ghz radio interface, valid for simultaneous multi radio network.
 */
#define PHY_INDEX_NATIVE        0

/**
 * @brief Phy index for SubGhz radio interface, valid for simultaneous multi radio network.
 */
