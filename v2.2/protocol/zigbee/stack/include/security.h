...
/** @brief An API to request a new link link key from the Trust Center. This
 ...
 *  API in order for it to be used by a joining device again.
 *
 * @param partnerEUI64 The EUI of the joining device. If all FF's are entered
 * for this value, then the key will be able to be used for all joining devices
...
/** @brief Clear all of the transient link keys from RAM. */
...
/*
 * @brief: returns the current rejoin policy in place, either it is allowing no rejoin
 * at all, allowing rejoin only with non-default link keys, or allowing all kinds of rejoins
...
/*
 * @brief: Sets the current rejoin policy in place, either it is allowing no rejoin
 * at all, allowing rejoin only with non-default link keys, or allowing all kinds of
 * rejoins.
 *
