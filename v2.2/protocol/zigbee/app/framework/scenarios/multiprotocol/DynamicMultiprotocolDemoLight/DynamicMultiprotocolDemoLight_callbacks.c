...
static void readSourceAddress(uint8_t connection)
{
  ...
  /* Send response to read request */
  gecko_cmd_gatt_server_send_user_read_response(connection, gattdb_source_address, 0,
...
static void writeLightState(uint8_t connection, uint8array *writeValue)
{
  ...
  if (index != 0xFF) {
    ...
    for (int i = 0; i < SOURCE_ADDRESS_LEN - 2; i++) {
      SourceAddress[2 + i] = bleConnectionTable[index].remoteAddress[5 - i];
    }
  }
}
...
static void notifyLight(uint8_t lightState)
{
  ...
  if (ble_lightState_config == GAT_RECEIVE_INDICATION) {
    ...
    for ( int i = 0; i < EMBER_AF_PLUGIN_BLE_MAX_CONNECTIONS; i++) {
      if (bleConnectionTable[i].inUse && bleConnectionTable[i].connectionHandle) {
        gecko_cmd_gatt_server_send_characteristic_notification(bleConnectionTable[i].connectionHandle, gattdb_light_state, sizeof(lightState), &lightState);
      }
    }
  }
}
...
static void notifySourceAddress(uint8_t connection)
{
  if (ble_triggerSrc_config == GAT_RECEIVE_INDICATION) {
    /* Send notification/indication data */
    gecko_cmd_gatt_server_send_characteristic_notification(connection, gattdb_source_address, sizeof(SourceAddress), SourceAddress);
  }
}
...
{
  if ((cmd->commandId == ZCL_ON_COMMAND_ID) || (cmd->commandId == ZCL_OFF_COMMAND_ID)
      || (cmd->commandId == ZCL_TOGGLE_COMMAND_ID)) {
    memset(SwitchEUI, 0, SOURCE_ADDRESS_LEN);
    emberLookupEui64ByNodeId(cmd->source, SwitchEUI);
    emberAfCorePrintln("SWITCH ZCL toggle/on/off EUI [%x %x %x %x %x %x %x %x]",
                       SwitchEUI[7], SwitchEUI[6],
                       SwitchEUI[5], SwitchEUI[4], SwitchEUI[3],
                       SwitchEUI[2], SwitchEUI[1], SwitchEUI[0]);
  }
  return false;
}
