...
// *
// * This plugin is used for testing. It creates MAC address filters to
// * artifically drop packets from certain devices to force a multi-hop
...
static FilterResult parseAndMaybeFilterPacket(EmberMessageBuffer rawMacHeader,
                                              uint8_t macFrameControlStartIndex)
{
  ...
  uint16_t frameControl = (emberGetLinkedBuffersByte(rawMacHeader,
                                                     macFrameControlStartIndex)
                           + ((emberGetLinkedBuffersByte(rawMacHeader,
                                                         macFrameControlStartIndex + 1))
                              << 8));
  ...
  switch (frameType) {
    ...
    case MAC_FRAME_CONTROL_FRAME_TYPE_COMMAND: {
      ...
      if (((destAddressModeFrameControl == MAC_FRAME_CONTROL_ADDRESS_MODE_NONE)
        ...
        panId = (emberGetLinkedBuffersByte(rawMacHeader, index)
                 + (emberGetLinkedBuffersByte(rawMacHeader, index + 1) << 8));
      ...
      if (sourceMode == MAC_ADDRESS_MODE_SHORT) {
        macSourceShortAddress = (emberGetLinkedBuffersByte(rawMacHeader, index)
                                 + (emberGetLinkedBuffersByte(rawMacHeader,
                                                              index + 1) << 8));
      } else if (sourceMode == MAC_ADDRESS_MODE_LONG) {
        emberCopyFromLinkedBuffers(rawMacHeader, index, macSourceLongAddress, EUI64_SIZE);
      }
...
// DECISION POINT:  Either we have to change this API to return a value,
// or we have to override the void* for a RAW MAC frame to return a value,
// or we have to create a new API.
// I am opting for the middle option.
                                 ...
                                 void *data)
{
  ...
  FilterResult result;
  if (commandType != EMBER_ZIGBEE_COMMAND_TYPE_RAW_MAC) {
    expectCheckpoint("Ignoring non-matching callback type.");
    return;
  }

  *returnValue = true; // accept packet
  macAddressFilterData.totalPackets++;

  result = parseAndMaybeFilterPacket(commandBuffer, indexOfCommand);
  if (result == DROP_PACKET) {
    *returnValue = false; // false = drop packet
  }
}
