...
#endif
static bool rxAlwaysOn = alwaysOn;
...
static void initFactoryNew(void)
{
  ...
  if (amFactoryNew()) {
    ...
    // scan requests.
    emberZllSetRxOnWhenIdle(rxAlwaysOn ? 0xFFFFFFFF
                            : EMBER_AF_PLUGIN_ZLL_COMMISSIONING_RX_ON_AT_STARTUP_PERIOD * MILLISECOND_TICKS_PER_SECOND);
...
{
  debugPrintln("startScan: purpose = %X, flags = %2X", purpose, flags);
...
static EmberStatus formNetworkForRouterInitiator(uint8_t channel)
{
  EmberStatus status;
...
static boolean isTouchlinkPermitted(const EmberZllNetwork *networkInfo)
{
  ...
  if (scanForReset() || scanForTouchLink()) {
    ...
    if (status != EMBER_NOT_JOINED
        && MEMCOMPARE(securityState.trustCenterLongAddress, allZeroEui64, EUI64_SIZE)
  ...
  if (touchLinkInProgress()) {
    ...
    if (scanForTouchLink()) {
      ...
      if (!amFactoryNew()) {
        ...
        status = formNetworkForRouterInitiator(network.zigbeeNetwork.channel);
        if (status != EMBER_SUCCESS) {
          emberAfAppPrintln("%p%p%p0x%x",
                            "Error: ",
                            "Touch linking failed: ",
...
static uint32_t getChannelMask(uint8_t purpose)
{
  ...
  if (emAfZllSecondaryChannelMask != 0) {
    // We may always scan the secondary channels if configured to do so.
