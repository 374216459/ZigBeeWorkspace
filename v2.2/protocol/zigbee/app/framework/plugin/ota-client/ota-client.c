...
static PGM uint8_t imageNotifyPayloadLengths[] = {
...
typedef enum {
  DISABLE_FEATURE = 0,    // Do not use Minimum Block Period feature
  MILLISECONDS    = 1,    // HA and SE1.2a (and prior) interpretation of MBP
  SECONDS         = SECONDS_IN_MS,  // (D) SE1.2 interpretation of MBP
...
#else
  #define MIN_BLOCK_PERIOD_UNITS SECONDS
#endif
...
void emAfOtaClientServiceDiscoveryCallback(const EmberAfServiceDiscoveryResult *result)
{
  ...
  if (result->status == EMBER_AF_BROADCAST_SERVICE_DISCOVERY_COMPLETE) {
    ...
    if (epList->count > 0 && serverEndpoint == UNDEFINED_ENDPOINT) {
      ...
      // In testing, we may want to ignore any OTA server that is not the TC
      if (result->matchAddress != 0x0000) {
                                      ...
                                      uint8_t length)
{
  ...
  if (queryJitter < 100) {
    uint8_t random = (((uint8_t)halCommonGetRandom()) % 100) + 1;
...
static bool downloadAndVerifyFinish(EmberAfOtaDownloadResult result)
{
  static PGM uint8_t zclStatusFromResult[] = {
    EMBER_ZCL_STATUS_SUCCESS,
