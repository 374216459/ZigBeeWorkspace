...
{
  EzspStatus status;
  uint8_t responseFrameId;
  uint8_t responseFrameControl;
  uint8_t responseExtendedFrameControl;
  uint8_t frameIdIndex = EZSP_FRAME_ID_INDEX;
  ...
  if (serialGetResponseByte(EZSP_FRAME_ID_INDEX) == EZSP_EXTENDED_FRAME_ID) {
    frameIdIndex = EZSP_EXTENDED_FRAME_ID_INDEX;
    parametersIndex = EZSP_EXTENDED_PARAMETERS_INDEX;
  }

  if (status == EZSP_SUCCESS) {
    responseFrameControl = serialGetResponseByte(EZSP_FRAME_CONTROL_INDEX);
    responseFrameId = serialGetResponseByte(frameIdIndex);
  ...
  if (status == EZSP_SUCCESS) {
    // if extended frame, use the correct indices for frame ID and parameters
    if (serialGetResponseByte(EZSP_FRAME_ID_INDEX) == EZSP_EXTENDED_FRAME_ID) {
      responseExtendedFrameControl = serialGetResponseByte(EZSP_EXTENDED_FRAME_CONTROL_INDEX);

      if (responseExtendedFrameControl & EZSP_EXTENDED_FRAME_CONTROL_RESERVED_MASK) {
        // reject if unsupported frame
        status = EZSP_ERROR_UNSUPPORTED_CONTROL;
      } else if ((responseExtendedFrameControl & EZSP_EXTENDED_FRAME_CONTROL_SECURITY_MASK)
...
static EzspStatus sendCommand(void)
{
  ...
  // Send initial EZSP_VERSION command with old packet format for old Hosts/NCPs
  if (serialGetResponseByte(EZSP_FRAME_ID_INDEX) == EZSP_VERSION) {
    initialEzspVersionSent = true;
  } else {
    if (emSecureEzspIsOn()
        && serialGetResponseByte(EZSP_EXTENDED_FRAME_ID_INDEX) != EZSP_RESET_TO_FACTORY_DEFAULTS) {
      serialSetCommandByte(EZSP_EXTENDED_FRAME_CONTROL_INDEX,
                           EZSP_EXTENDED_FRAME_CONTROL_SECURE);
      emSecureEzspEncode();
    } else {
      serialSetCommandByte(EZSP_EXTENDED_FRAME_CONTROL_INDEX, 0x00);
    }
  }
...
/** @brief This call will update ZLL security token information. Unlike
...
{
  return ezspZllSetSecurityStateWithoutKey((EmberZllInitialSecurityState *) securityState);
}
