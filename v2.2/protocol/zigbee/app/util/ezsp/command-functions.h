...
{
  uint8_t zllOperationInProgress;
...
{
  uint8_t zllRxOnWhenIdleGetActive;
...
static void callbackDispatch(void)
{
  ...
  switch (serialGetResponseByte(EZSP_EXTENDED_FRAME_ID_INDEX)) {
    ...
    case EZSP_CHANGE_SOURCE_ROUTE_HANDLER: {
      uint16_t newChildId;
      uint16_t newParentId;
      uint8_t ourChild;
      newChildId = fetchInt16u();
      newParentId = fetchInt16u();
      ourChild = fetchInt8u();
      ezspChangeSourceRouteHandler(newChildId, newParentId, ourChild);
      break;
    }
    ...
    case EZSP_GPEP_INCOMING_MESSAGE_HANDLER: {
      ...
      ezspGpepIncomingMessageHandler(status, gpdLink, sequenceNumber, &addr, gpdfSecurityLevel, gpdfSecurityKeyType, autoCommissioning, rxAfterTx, gpdSecurityFrameCounter, gpdCommandId, mic, proxyTableIndex, gpdCommandPayloadLength, gpdCommandPayload);
      break;
    }
